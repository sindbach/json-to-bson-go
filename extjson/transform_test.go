...
func TestConvert(t *testing.T) {
	...
	testCases := []struct {
		...
		{"scalar values", "extjson_scalar", "simplejson_scalar", nil, false},
		{"primitive values", "extjson_primitive", "primitive", nil, false},
		{"arrays", "extjson_array", "simplejson_array", nil, false},
		{"nested documents", "extjson_nested", "simplejson_nested", nil, false},
		...
		{"minimize ints true", "extjson_scalar", "simplejson_scalar", minimizeTrueOpts, false},
		{"minimize ints false", "extjson_minimize_false", "simplejson_minimize_false", minimizeFalseOpts, false},
		{"truncate ints true is a noop if minimize is false", "extjson_minimize_false", "simplejson_minimize_false", truncateTrueMinFalseOpts, false},
		{"truncate true", "extjson_minimize_false", "simplejson_truncate_true", truncateTrueOpts, false},
		{"truncate false", "extjson_scalar", "simplejson_scalar", truncateFalseOpts, false},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			input := jsonutil.ReadFileBytesOrPanic(fmt.Sprintf("../testdata/%s.json", tc.inputfile))
			actual, err := Convert(input, false, tc.opts)
			if tc.expectErr {
				if err == nil {
